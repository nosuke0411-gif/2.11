...
ITEM_DESCRIPTIONS = {
    ...
    "特製おやつ": {"emoji": "🧺", "description": "探検中に使うと、残り時間を60分短縮できるよ！"},
    "祈りの結晶": {"emoji": "🔮", "description": "次の探検でEXP +50%、発見率 +20%！幻のエリアが出るかも…？"},
    "金のコイン": {"emoji": "🪙✨", "description": "使うと500コインが手に入るよ！"},
    "神秘の面": {"emoji": "🦊🔥", "description": "スーパーチャーム +2、次の探検でEXPアップ！幻のキツネが出るかも？"}
}
# --- JSONファイル読み書き ---
...
async def combine_items(interaction: discord.Interaction):
    await interaction.response.defer(thinking=True)
    user_id = str(interaction.user.id)
    items = get_user_items(user_id)
    ...
    await interaction.followup.send("⚗️ 合成できるアイテムの組み合わせが見つからなかったよ！")
...
async def item_info(
    ...
    if item_name:
        ...
        await interaction.followup.send(f"{emoji} **{item_name}** の効果：\n{description}")
    else:
        lines = [
